...
class GridDialog extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?Editor $editor = NULL) {
    ...
    if ($editor instanceof EditorInterface) {
      ...
      if (isset($editor_settings['plugins']['ckeditor_bs_grid_grid'])) {
  ...
  public function buildLayoutStep(array $form, FormStateInterface $form_state) {
    ...
    foreach ($config->get('breakpoints') as $class => $breakpoint) {
      ...
      // Add layout options first (excluding 'none').
      foreach ($breakpoint['columns'][$num_cols]['layouts'] as $layout) {
        $key = implode('_', $layout['settings']);
        $options[$key] = $layout['label'];
      }

      // Move 'none' to the bottom.
      $options['none'] = $this->t('None (advanced)');

      // Determine the default layout.
      if (!empty($settings['breakpoints'][$prefix]['layout'])) {
        $default_layout = $settings['breakpoints'][$prefix]['layout'];
      }
      elseif (array_key_exists('equal', $options)) {
        $default_layout = 'equal';
      }
      else {
        // Fallback to the first available (excluding 'none') if present.
        $default_layout = array_key_first(array_diff_key($options, ['none' => TRUE])) ?? 'none';
      }
